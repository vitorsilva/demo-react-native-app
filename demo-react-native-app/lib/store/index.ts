...
interface StoreState {
  ...
  /** All pairing rules (positive = pairs well, negative = avoid together) */
  ...
  // ─── Pairing Rules Actions ─────────────────────────────────────────────────
  /** Loads all pairing rules from the database into state */
  loadPairingRules: () => Promise<void>;
  /** Adds a new pairing rule */
  ...
  deletePairingRule: (id: string) => Promise<{ success: boolean; error?: string }>;
  /** Gets all pairing rules involving a specific ingredient (from state) */
...
export const useStore = create<StoreState>((set, get) => ({
  ...
  // Action: Load all pairing rules
  loadPairingRules: async () => {
    set({ isLoading: true, error: null });
    try {
      const db = getDatabase();
      const pairingRules = await pairingRulesDb.getAllPairingRules(db);
      set({ pairingRules, isLoading: false });
    } catch (error) {
      set({
        error: error instanceof Error ? error.message : 'Failed to load pairing rules',
        isLoading: false,
      });
    }
  },

  // Action: Add a new pairing rule
  addPairingRule: async (ingredientAId, ingredientBId, ruleType) => {
    set({ isLoading: true, error: null });
    try {
      const db = getDatabase();
      const newRule = await pairingRulesDb.addPairingRule(db, ingredientAId, ingredientBId, ruleType);
      set((state) => ({
        pairingRules: [...state.pairingRules, newRule],
        isLoading: false,
      }));
      return newRule;
    } catch (error) {
      set({
        error: error instanceof Error ? error.message : 'Failed to add pairing rule',
        isLoading: false,
      });
      throw error;
    }
  ...
  // Action: Delete a pairing rule
  deletePairingRule: async (id) => {
    set({ isLoading: true, error: null });
    try {
      const db = getDatabase();
      const result = await pairingRulesDb.deletePairingRule(db, id);
      if (result.success) {
        set((state) => ({
          pairingRules: state.pairingRules.filter((rule) => rule.id !== id),
          isLoading: false,
        }));
      } else {
        set({ error: result.error, isLoading: false });
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to delete pairing rule';
      set({ error: errorMsg, isLoading: false });
      return { success: false, error: errorMsg };
    }
  },

  // Action: Get pairing rules for a specific ingredient (from state)
  getPairingRulesForIngredient: (ingredientId) => {
    const { pairingRules } = get();
    return pairingRules.filter(
      (rule) => rule.ingredientAId === ingredientId || rule.ingredientBId === ingredientId
    );
  },
